...
class gtr1 : public treatment_machine_ion<T> {
   ...
   public:
    ...
    gtr1() {
        ...
        std::vector<double> beamEnergyForParticleCountCalib = {
            70,  76,  84,  92,  100, 106, 112, 118, 126, 134, 142, 146, 150, 154, 158,
            162, 170, 174, 178, 182, 186, 190, 198, 202, 206, 214, 218, 222, 226, 230};
        std::vector<double> correctionParticleCountCalib = {
            1,           1.065789513, 1.157943145, 1.2411556,   1.322953524, 1.395086046,
            1.458739464, 1.513003859, 1.588552029, 1.66206693,  1.740579354, 1.780360004,
            1.81434049,  1.848968773, 1.878347883, 1.91131038,  1.971904065, 2.003357087,
            2.02810976,  2.058108192, 2.082444044, 2.108758102, 2.168636068, 2.20480403,
            2.229879571, 2.308058176, 2.342322973, 2.381755055, 2.447585594, 2.511318715};
        particleCountCalibInterp.set_points(beamEnergyForParticleCountCalib,
                                            correctionParticleCountCalib, tk::spline::cspline);
        ...
        std::vector<double> correctionBeamSpotSize = {
            10.70, 9.94, 9.10, 8.45, 7.88, 7.52, 7.18, 6.88, 6.52, 6.22,
            5.94,  5.82, 5.70, 5.57, 5.47, 5.37, 5.16, 5.05, 4.94, 4.86,
            4.76,  4.66, 4.50, 4.42, 4.34, 4.22, 4.15, 4.03, 3.85, 3.58};
        ...
        std::vector<double> correctionBeamAngularSpread = {
            0.004675, 0.00424,  0.00379,  0.003605, 0.003315, 0.003045, 0.00294,  0.0028,
            0.00253,  0.002435, 0.002285, 0.002205, 0.002295, 0.00224,  0.002075, 0.002155,
            0.002075, 0.001865, 0.00196,  0.001855, 0.001885, 0.00195,  0.00205,  0.001945,
            0.002,    0.00184,  0.0018,   0.001795, 0.00171,  0.001695};
        ...
        std::vector<double> correctionBeamDivergence = {
            0.0871,  0.0736,  0.0606,  0.0515,  0.0438,  0.0389,  0.0347, 0.0311,  0.027,  0.0236,
            0.0206,  0.0193,  0.018,   0.0164,  0.0154,  0.0144,  0.0124, 0.0116,  0.0109, 0.0104,
            0.00949, 0.00892, 0.00799, 0.00766, 0.00739, 0.00671, 0.0065, 0.00638, 0.0066, 0.00697};
    ...
    size_t characterize_history(const mqi::beam_module_ion::logspot& s) {
        ...
        int particleFromMUCount =
            s.muCount * particleCountCalibInterp(
                            s.e);  // * protonPerDoseInterp(s.e) * dosePerMUCountInterp(s.e);
    ...
    mqi::beamlet<T> characterize_beamlet(const mqi::beam_module_ion::logspot& s,
        ...
        if (rsuse)
            newBeamStartingPos = newBeamStartingPos - 50;
    ...
    mqi::rangeshifter* characterize_rangeshifter(const mqi::dataset* ds, mqi::modality_type m) {
        ...
        for (auto r : rs_ds) {
            ...
            if (rs_id[0] == "SNOUT_DEG_B" || rs_id[0] == "SNOUT_DEG_S")
                lxyz.z = 41.0;
            else
                lxyz.z = 0.0;
            assert(lxyz.z > 0);
