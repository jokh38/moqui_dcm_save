...
namespace mc {
template <typename R>
CUDA_GLOBAL void print_materials(mqi::material_t<R>* m, uint16_t n_m);
template <typename R>
CUDA_DEVICE void print_node_specification_gpu(mqi::node_t<R>* node);
template <typename R>
CUDA_GLOBAL void print_node_specification(mqi::node_t<R>* node);
...
#if defined(__CUDACC__)
CUDA_GLOBAL
void print_density(__half* g_density, uint32_t size_) {
...
CUDA_GLOBAL
void print_density(float* g_density, uint32_t size_) {
    for (int i = 0; i < size_; i++) {
        printf("g_density %d %.10f\n", i, g_density[i]);
    }
}
