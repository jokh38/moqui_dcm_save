...
class x_environment {
   public:
    ...
    /// 1. setup geometry
    CUDA_HOST
    virtual void setup_world() = 0;

    /// 2. setup materials
    CUDA_HOST
    virtual void setup_materials() = 0;

    /// 3. setup beam source
    CUDA_HOST
    virtual void setup_beamsource() = 0;
    ...
    void save_reshaped_files() {
        ...
        for (int c_ind = 0; c_ind < this->world->n_children; c_ind++) {
            for (int s_ind = 0; s_ind < this->world->children[c_ind]->n_scorers; s_ind++) {
                filename = std::to_string(c_ind) + "_" +
                ...
                if (!this->output_format.compare("mhd")) {
                    mqi::io::save_to_mhd<R>(this->world->children[c_ind], reshaped_data, 1.0,
                                            this->output_path, filename, vol_size);
                } else if (!this->output_format.compare("mha")) {
                    mqi::io::save_to_mha<R>(this->world->children[c_ind], reshaped_data, 1.0,
                                            this->output_path, filename, vol_size);
                } else {
                    mqi::io::save_to_bin<double>(reshaped_data, 1.0, this->output_path, filename,
                                                 vol_size);
                }
        ...
        // duration = stop - start; printf("Reshape and save done %f ms\n", duration.count());
        ...
        for (int c_ind = 0; c_ind < this->world->n_children; c_ind++) {
            for (int s_ind = 0; s_ind < this->world->children[c_ind]->n_scorers; s_ind++) {
                ...
                filename = std::to_string(c_ind) + "_" +
                           this->world->children[c_ind]->scorers[s_ind]->name_;
#if defined(__CUDACC__)
                mqi::io::save_to_npz<R>(this->world->children[c_ind]->scorers[s_ind], 1.0,
                                        this->output_path, filename, dim, this->num_spots);
#else
                mqi::io::save_to_npz<R>(this->world->children[c_ind]->scorers[s_ind], 1.0,
                                        this->output_path, filename, dim, this->num_spots);
#endif
...
#if defined(__CUDACC__)
    CUDA_HOST
    void calculate_memory_expectation() {
        size_t free, total, mem = 0;
        cudaMemGetInfo(&free, &total);
        printf("total memory %lu MB Free memory %lu MB\n", total / (1024 * 1024),
               free / (1024 * 1024));
