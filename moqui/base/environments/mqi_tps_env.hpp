...
class tps_env : public x_environment<R> {
   ...
   public:
    ...
    tps_env(const std::string input_name) : x_environment<R>() {
        ...
        if (master_seed < -1)
            std::cout << "Reading seeds.. : Using negative value generates current time based "
                         "random seed."
                      << std::endl;
        ...
        if (use_absolute_path) {
            ...
            this->dicom_dir = parser.get_string(
                "logFilePath", "");  // Log file dir added in 2023-11-01 by Chanil Jeon
        } else {
            this->parent_dir = parser.get_string("ParentDir", "");
        ...
        if (dicom_dir.empty()) {
            throw std::runtime_error("DICOM Dir is not provided");
        }
        if (logfile_dir.empty()) {
            throw std::runtime_error("Log Dir is not provided");
        }
        ...
        if (this->scorer_type == mqi::DOSE_Dij) {
        ...
        // Masking file
        if (mask_filenames.size() > 0)
            std::cout << "Reading masking file.. : Masking file count --> " << mask_filenames.size()
                      << std::endl;
        else
        ...
        if (output_path.empty()) {
            ...
            std::cout << "Setting output.. : Output directory name --> " << output_path
                      << std::endl;
        ...
        tx = new mqi::treatment_session<R>(dcm_.plan_name, machineName, referenceMCName,
        ...
        if (sim_type == mqi::PER_BEAM) {
            ...
            if (beam_numbers.size() == 0) {
                for (int k = 1; k < this->tx->get_num_beams() + 1; k++) {
                    ...
                    if (this->tx->get_beam_name(k) == "Setup")
                        continue;
    ...
    CUDA_HOST
    virtual struct dicom_t read_dcm_dir() {
        ...
        // Fix: Use custom file discovery instead of GDCM Directory::Explore()
        std::vector<std::string> dicom_files;
        std::string find_cmd = "find '" + dicom_dir + "' -maxdepth 1 -name '*.dcm' 2>/dev/null";
        ...
        if (pipe) {
            ...
            while (fgets(buffer, sizeof(buffer), pipe) != nullptr) {
                ...
                if (!file_path.empty() && file_path.back() == '\n') {
                    file_path.pop_back();  // Remove newline
                }
                if (!file_path.empty()) {
                    dicom_files.push_back(file_path);
                }
            }
            pclose(pipe);
        }

        std::cout << "Reading DICOM directory.. : DICOM directory name --> " << dicom_dir
                  << std::endl;
        std::cout << "Found " << dicom_files.size() << " DICOM files" << std::endl;

        if (dicom_files.empty()) {
            throw std::runtime_error("No DICOM files found in directory: " + dicom_dir);
        }

        dcm.nfiles = dicom_files.size();
        gdcm::Scanner s0;

        // Reading modality tag of DICOM
        const gdcm::Tag Modality(0x0008, 0x0060);  // Modality
        s0.AddTag(Modality);
        bool b = s0.Scan(dicom_files);
        if (!b) {
        ...
        if (!this->usingPhantomGeo) {
            std::cout << "Patient geometry mode: Loading CT data..." << std::endl;

            // Check if CT files exist before attempting to load
            std::vector<std::string> ct_files;
            for (const auto& file : dicom_files) {
                // Use filename pattern to detect CT files (simple approach)
                std::string filename = file.substr(file.find_last_of("/\\") + 1);
                std::transform(filename.begin(), filename.end(), filename.begin(), ::toupper);

                // Common CT file naming patterns
                if (filename.find("CT") != std::string::npos ||
                    filename.rfind(".DCM", filename.length() - 4) != std::string::npos) {
                    // Additional check using GDCM to read modality tag
                    gdcm::Reader reader;
                    reader.SetFileName(file.c_str());
                    if (reader.Read()) {
                        gdcm::StringFilter sf;
                        sf.SetFile(reader.GetFile());
                        std::string modality = sf.ToStringPair(gdcm::Tag(0x0008, 0x0060)).second;
                        if (modality == "CT") {
                            ct_files.push_back(file);
                        }
                    }
                }
            }
            ...
            if (ct_files.empty()) {
                std::cerr << "ERROR: Dicom CT data is not loaded" << std::endl;
                std::cerr << "No CT files found in directory: " << dicom_dir << std::endl;
                std::cerr << "Please ensure CT DICOM files are present or use UsingPhantomGeo true"
                          << std::endl;
                throw std::runtime_error("Dicom CT data is not loaded - No CT files found");
            }

            std::cout << "Found " << ct_files.size() << " CT files, loading CT data..."
                      << std::endl;
            ...
            std::cout << "CT geometry loaded: " << dcm.dim_.x << "x" << dcm.dim_.y << "x"
                      << dcm.dim_.z << " voxels" << std::endl;
        } else {
            std::cout << "Phantom geometry mode: Using water phantom (" << this->phantomDimX << "x"
                      << this->phantomDimY << "x" << this->phantomDimZ << " voxels)" << std::endl;
            ...
            for (int i = 0; i < this->phantomDimZ; i++)
                dcm.org_dz[i] = this->phantomUnitZ;
        }
        ...
        std::cout << "Reading DICOM directory.. : Loading RT Ion plan data from " << dcm.plan_name
                  << " .." << std::endl;
            ...
            for (int i = 0; i < dcm.dim_.z + 1; i++) {
                if (i == 0)
                    ...
                    dcm.org_ze[i] =
                        dcm.org_ze[i - 1] +
                        dcm.org_dz[i -
                                   1];  // need to be modified to deal with varying slice thickness
            }
        ...
        if (dcm.n_struct >= 1 && this->read_structure) {
            ...
            if (!is_valid_rs)
                throw std::runtime_error("Invalid RT structure file.");
            ...
            for (int con_ind = 0; con_ind < roi_contour_seq.size(); con_ind++) {
                ...
                if (refer_roi[0] == body_ind[0]) {
                    ...
                    for (int contour_ind = 0; contour_ind < contour_seq.size(); contour_ind++) {
                        ...
                        fill_contour(body_contour, contour_points, contour_num[0], dcm.dim_, dcm.xe,
                                     dcm.ye, dcm.ze, dcm.dx, dcm.dy);
    ...
    CUDA_HOST
    virtual struct logfiles_t read_logfile_dir(int beamIndex) {
        ...
        for (int i = 0; i < fileFolderName.size(); i++) {
            if (i == beamIndex) {
                ...
                for (auto& filename : sortedByName) {
                    ...
                    if (filename.extension() == ".csv") {
                        ...
                        logBeamEnergyString = logBeamEnergyString.substr(
                            logBeamEnergyString.find('_') + 1, logBeamEnergyString.find('M') - 2);
                        float layerBeamEnergy = std::stof(logBeamEnergyString);
                        fieldBeamEnergy.push_back(layerBeamEnergy);

                        std::cout << "Reading log file information in the field directory.. : " +
                                         filename.string() + ".."
                                  << std::endl;
                        ...
                        while (!fs.eof()) {
                            if (std::getline(fs, strBuf, ',')) {
                                ...
                                if (csvIndex == 2)
                                    selectedLogFiledInfo.posX.push_back(stof(strBuf));
                                else if (csvIndex == 3)
                                    selectedLogFiledInfo.posY.push_back(stof(strBuf));
                                else if (csvIndex == 4) {
                                    selectedLogFiledInfo.muCount.push_back(
                                        stoi(strBuf) * this->particles_per_history);
                                    csvIndex = 0;
                                }
                            }
                        }
                ...
                std::cout << "Reading log file information for beam " << beamIndex + 1
    ...
    virtual void setup_world() {
        ...
        this->world->geo->fill_data(
            0.0);  // There is no meaning that add some density to world geometry
        ...
        this->world->n_children =
            beamline_geometries.size() + 1 +
            worldChildCorrection;  // Original is + 1. Additional + 1 for air box
        this->world->children =
            new node_t<R>*[this->world->n_children];  // Beam line geometry + airbox
        ...
        if (this->usingPhantomGeo) {
            ...
            // Set air box to end index of beamline object, becuase the simulation calculates
            // sequentially
            ...
            airBox->geo =
                new grid3d<density_t, R>(-200, 200, 2, -200, 200, 2, airEndPos,
        ...
        if (!this->usingPhantomGeo) {
            ...
            // mqi::material_id* mids = new mqi::material_id[dcm_.dim_.x * dcm_.dim_.y *
            // dcm_.dim_.z];
            phantom->geo = new grid3d<density_t, R>(this->dcm_.xe, this->dcm_.dim_.x + 1,
                                                    this->dcm_.ye, this->dcm_.dim_.y + 1,
                                                    this->dcm_.ze, this->dcm_.dim_.z + 1);
        ...
        } else                                 // 2. If user uses phantom geometry
        {
            ...
            if (this->twoCentimeterMode) {
                ...
                frontPhantom->geo = new grid3d<density_t, R>(-200, 200, 401, -200, 200, 401, 1, 20,
                ...
                backPhantom->geo = new grid3d<density_t, R>(-200, 200, 401, -200, 200, 401, -380,
                ...
                phantom->geo = new grid3d<density_t, R>(
                    this->dcm_.xe, this->dcm_.dim_.x + 1, this->dcm_.ye, this->dcm_.dim_.y + 1,
                    this->dcm_.ze, this->dcm_.dim_.z + 1, transformPhantom.rotation);
    ...
    CUDA_HOST
    virtual void setup_beamsource() {
        uint16_t num_beams = this->tx->get_num_beams();
        std::vector<std::string> beam_names = this->tx->get_beam_names();
        std::cout << "Creating beam source object.. : Detected beam count --> " << num_beams
                  << std::endl;
        std::cout << "Creating beam source object.. : Currently selected beam --> " << bnb << " ("
                  << beam_names[bnb - 1] << ")" << std::endl;
        ...
        this->sid = temp_sid[0];
        std::cout << "Creating beam source object.. : Detected snout position --> " << this->sid
                  << " mm" << std::endl;
        mqi::coordinate_transform<R> p_coord = this->tx->get_coordinate(bnb);
        p_coord.angles[3] = 90.0;  // iec2dicom angle
        ...
        std::cout << "Creating coordinate transform for beam source complete.. : p_coord angle "
                     "information -- >"
                  << std::endl;
        std::cout << "(" << p_coord.angles[0] << ", " << p_coord.angles[1] << ", "
                  << p_coord.angles[2] << ", " << p_coord.angles[3] << ")" << std::endl;
        ...
        if (rangeShifterCount[0] >= 1)
            rangeShifterUsed = true;

        std::cout << "Creating coordinate transform for beam source complete.. : Translation "
                     "information -- > "
                  << std::endl;
        p_final.translation.dump();
        std::cout << "Creating coordinate transform for beam source complete.. : Rotation "
                     "information -- > "
                  << std::endl;
        ...
        std::cout << "Total histories (Particles) : " << this->beamsource.total_histories()
                  << std::endl;
    ...
    virtual void run() {
        ...
        if (this->sim_type == mqi::PER_BEAM) {
            ...
            std::cout << "Starting simulation per field.. : Geometry allocated memory --> "
                      << (total - free) / (1024.0 * 1024.0 * 1024.0) << " GB" << std::endl;
#endif
            run_by_beam();
    ...
    CUDA_HOST
    virtual void run_simulation(size_t histories_per_batch, size_t histories_in_batch,
        ...
        /// the histories_per_batch may not required if copying memory work correctly with
        /// histories_in_batch
        ...
        // auto                                      stop =
        // std::chrono::high_resolution_clock::now(); std::chrono::duration<double, std::milli>
        // duration = stop - start; printf("Initialization for run done %f ms\n", duration.count());
#if defined(__CUDACC__)
        // start     = std::chrono::high_resolution_clock::now();
        ...
        if (histories_in_batch < n_threads) {
            ...
            // assert(n_blocks*n_threads>histories_in_batch &&
            // (n_blocks-1)*n_threads<histories_in_batch);
        }
        ...
        if (scorer_offset_vector) {
            printf("Printing simulation specification.. : Histories per batch --> %d\n",
                   histories_per_batch);
            mc::upload_scorer_offset_vector(scorer_offset_vector, d_scorer_offset_vector,
                                            histories_per_batch);
            cudaDeviceSynchronize();
        } else {
        ...
        initialize_threads<<<n_blocks, n_threads>>>(worker_threads, n_blocks * n_threads,
                                                    this->master_seed);
        cudaDeviceSynchronize();
        check_cuda_last_error("(initialize threads)");
        gpu_err_chk(cudaMalloc(&d_tracked_particles, sizeof(tracked_particles[0])));
        gpu_err_chk(cudaMemcpy(d_tracked_particles, tracked_particles, sizeof(tracked_particles[0]),
        ...
        gpu_err_chk(cudaMemcpy(tracked_particles, d_tracked_particles, sizeof(tracked_particles[0]),
        ...
        mc::transport_particles_patient<R>(worker_threads, mc::mc_world, mc::mc_vertices,
                                           histories_in_batch, tracked_particles);
#endif
    }  // run_simulation

    CUDA_HOST
    void read_vertices_spot(size_t history_start, size_t history_end,
                            std::tuple<mqi::beamlet<R>, size_t, size_t> bl,
                            mqi::vertex_t<R>* vertices, uint32_t* score_offset_vector, int spot_ind,
    ...
    CUDA_HOST
    virtual void run_by_beam(mqi::node_t<R>* world = mc::mc_world) {
        //// Beam simulation
        ...
        if (this->max_histories_per_batch <= 0) {
            ...
            std::cout << "Uploading particles with no batch.. : Particle count --> "
                      << histories_per_batch << std::endl;
        } else {
            num_batches = (int)mqi::mqi_ceil((h1 - h0) * 1.0 / this->max_histories_per_batch);
            histories_per_batch = this->max_histories_per_batch;
            std::cout << "Uploading particles with batch.. : Particle count --> "
                      << histories_per_batch << " with " << num_batches << " batches" << std::endl;
        }
    ...
    CUDA_HOST
    virtual void run_by_spot(mqi::node_t<R>* world = mc::mc_world) {
        ...
        while (spot_ind < this->num_spots) {
            ...
            for (spot_ind = spot_start; spot_ind < this->num_spots; spot_ind++) {
                ...
                read_vertices_spot(current_vertex, loop_end, bl, this->vertices,
                                   score_offset_vector, spot_ind, histories_per_batch);
        ...
        printf("spot ind %lu num_spots %d cum vertices %lu total histories %lu\n", spot_ind,
               this->num_spots, cum_vertices, h1);
        printf("Number of particles tracked %d\n", tracked_particles[0]);

    }  // run_by_spot

    virtual mqi::node_t<R>* create_rangeshifter(mqi::rangeshifter* geometry,
                                                mqi::coordinate_transform<R> p_coord) {
        ...
        printf(
            "Printing rangeshifter specification.. : p_coord angle of rangeshifter --> \n(%f %f %f "
            "%f)\n",
            p_coord.angles[0], p_coord.angles[1], p_coord.angles[2], p_coord.angles[3]);

        printf(
            "Printing rangeshifter specification.. : p_coord position of rangeshifter --> \n(%f %f "
            "%f)\n",
            p_coord.translation.x, p_coord.translation.y, p_coord.translation.z);
        ...
        rangeshifter->geo = new grid3d<mqi::density_t, R>(
            geometry->pos.x - geometry->volume.x / 2, geometry->pos.x + geometry->volume.x / 2, 2,
            geometry->pos.y - geometry->volume.y / 2, geometry->pos.y + geometry->volume.y / 2, 2,
            geometry->pos.z - geometry->volume.z / 2, geometry->pos.z + geometry->volume.z / 2, 2,
            p_final.rotation);
        ...
        // std::cout << "Printing rangeshifter specification.. : Rotation for coordinate transform
        // -->" << std::endl; p_final.rotation.dump();
        std::cout
            << "Printing rangeshifter specification.. : Rotation for rangeshifter geometry -->"
            << std::endl;
    ...
    CUDA_HOST_DEVICE
    bool sol1_1(mqi::vec2<float> pos, mqi::vec3<float>*& contour_points, int num_points) {
    ...
    CUDA_HOST_DEVICE
    void fill_contour(uint8_t* volume_contour, mqi::vec3<float>*& contour_points,
                      int num_contour_points, mqi::vec3<ijk_t> dim, const R* x_pix, const R* y_pix,
                      const R* z_pix, float dx, float dy) {
        ...
        for (int c_ind = 0; c_ind < this->world->n_children; c_ind++) {
            for (int s_ind = 0; s_ind < this->world->children[c_ind]->n_scorers; s_ind++) {
                ...
                mqi::io::save_to_npz<R>(this->world->children[c_ind]->scorers[s_ind],
                                        this->particles_per_history, this->output_path, filename,
                                        dim, this->num_spots);
            }
        }
        // auto                                      stop =
        // std::chrono::high_resolution_clock::now(); std::chrono::duration<double, std::milli>
        // duration = stop - start; printf("Reshape and save to npz done %f ms\n",
        // duration.count());
    }

    CUDA_HOST
    virtual void save_files() {
