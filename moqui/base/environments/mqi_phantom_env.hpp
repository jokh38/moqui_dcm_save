   ...
   public:
    ...
    virtual void setup_materials() {
        ...
        for (uint16_t i = 0; i < 2; ++i) {
            printf("%d: %f IeV, %f g/cm^3\n", i, this->materials[i].Iev,
                   this->materials[i].rho_mass);
        }
        //*/
    }
    ...
    virtual void setup_world() {
        ...
        phantom->geo = new grid3d<mqi::density_t, R>(x[0], x[1], nxyz.x + 1, y[0], y[1], nxyz.y + 1,
                                                     z[0], z[1], nxyz.z + 1);
    ...
    virtual void run() {
        ...
        gpu_err_chk(cudaMemcpy(d_tracked_particles, tracked_particles, sizeof(tracked_particles),
        ...
        gpu_err_chk(cudaMemcpy(tracked_particles, d_tracked_particles, sizeof(tracked_particles),
...
//            std::cout << "Number of particles tracked " << tracked_particles[0] << std::endl;
        ...
        mc::transport_particles_table<R>(worker_threads, mc::mc_world, mc::mc_vertices, h1,
                                         tracked_particles);
#endif
        std::cout << "Number of particles tracked " << tracked_particles[0] << std::endl;
