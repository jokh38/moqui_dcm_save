...
/// \file
///
/// A coordinate transform to map a history from beamsource to map a IEC coordinate or DICOM
/// coordinate.
...
class coordinate_transform {
   public:
    ...
    /// Assignment operator
    CUDA_HOST_DEVICE
    coordinate_transform<T>& operator=(const coordinate_transform<T>& ref) {
        collimator = ref.collimator;
        gantry = ref.gantry;
        patient_support = ref.patient_support;
        iec2dicom = ref.iec2dicom;
