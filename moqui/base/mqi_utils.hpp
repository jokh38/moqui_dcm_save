...
inline T interp_linear(const std::map<T, std::array<T, S>>& db, const T x, const size_t y_col = 0) {
...
/// \param y_col column index of map for y-values to be used for interpolation
template <typename T, size_t S>
inline T interp_linear(const std::vector<std::array<T, S>>& db, const T x, const size_t x_col = 0,
...
/// \note from gpmc code
inline float TableInterpolation(float* const vector_X, float* const vector_Y, const float x,
                                const int npoints, int order = 4) {
    float result;
...
CUDA_DEVICE
vec2<uint32_t> start_and_length(const uint32_t& n_threads, const uint32_t& n_jobs,
...
CUDA_HOST_DEVICE
int32_t lower_bound_cpp(const int32_t* arr, const int32_t& len, const int32_t& value) {
    int32_t first = 0;
    int32_t count = len;
    int32_t step;
