...
class treatment_session {
   ...
   public:
    ...
    treatment_session(std::string file_for_tx_machine,  // for beamline and source,
                      std::string m_name = "", std::string mc_code = "TOPAS 3.5",
                      int gantryNum = 2) {
    ...
    bool create_machine(std::string machine_name, std::string mc_code, int gantryNum) {
        if (tx_machine_)
        ...
        std::cout << "Creating treatment machine model.. : MC code version --> " << mc_code
                  << std::endl;
        std::cout << "Creating treatment machine model.. : Creating SMC treatment machine model.."
                  << std::endl;
    ...
    std::vector<std::string> get_beam_names() {
    ...
    std::string get_beam_name(int bnb) {
    ...
    const mqi::dataset* get_beam_dataset(std::string bnm) {
        auto bseq = (*mqi_ds_)(seq_tags_->at("beam"));
        for (auto i : bseq) {
            std::vector<std::string> bn;
    ...
    /// \return mqi::dataset* constant pointer.
    const mqi::dataset* get_beam_dataset(int bnb) {
        auto bseq = (*mqi_ds_)(seq_tags_->at("beam"));
        for (auto i : bseq) {
            // i->dump();
            std::vector<int> bn;
    ...
    template <typename S>
    mqi::beamline<T> get_beamline(S beam_id) {
        if (mtype_ == 1)
            std::cout << "Creating beam line object.. : RT Plan type --> RT ION" << std::endl;
    ...
    template <typename S>
    mqi::beamsource<T> get_beamsource(S beam_id, const mqi::coordinate_transform<T> coord,
                                      float scale, T sid) {
        return tx_machine_->create_beamsource(this->get_beam_dataset(beam_id), mtype_, coord, scale,
                                              sid);
    }
    ...
    /// \param scale   for calculating number of histories to be simulated from beamlet weight
    /// \return beamsource object.
    mqi::beamsource<T> get_beamsource(mqi::dataset* beam, const mqi::coordinate_transform<T> coord,
                                      float scale, T sid) {
    ...
    mqi::beamsource<T> get_beamsource(mqi::logfiles_t& logfileData,
                                      const mqi::coordinate_transform<T> coord, T sid,
                                      const bool rsuse) {
    ...
    /// \param beam_id
    template <typename S>
    std::map<T, int32_t> get_timeline(S beam_id) {
    ...
    template <typename S>
    mqi::coordinate_transform<T> get_coordinate(S beam_id) {
    ...
    /// Return ion type
    /// \return mqi::m_type_
    mqi::modality_type get_modality_type(void) { return mtype_; }

    const gdcm::DataElement& get_dataelement(gdcm::Tag tag) { return mqi_ds_[0][tag]; }
};
