...
template <typename T>
CUDA_DEVICE T mqi_ln(T s);

template <typename T>
CUDA_HOST_DEVICE T mqi_sqrt(T s);

template <typename T>
CUDA_DEVICE T mqi_pow(T s, T p);

template <typename T>
CUDA_DEVICE T mqi_exp(T s);

template <typename T>
CUDA_DEVICE T mqi_acos(T s);

template <typename T>
CUDA_DEVICE T mqi_cos(T s);

template <typename T>
CUDA_DEVICE T mqi_sin(T s);

template <typename T>
CUDA_DEVICE T mqi_abs(T s);

template <typename T>
CUDA_HOST_DEVICE T mqi_round(T s);

template <typename T>
CUDA_HOST_DEVICE T mqi_floor(T s);

template <typename T>
CUDA_HOST_DEVICE T mqi_ceil(T s);

template <typename T>
CUDA_HOST_DEVICE bool mqi_isnan(T s);
...
// Nautual log
template <>
float mqi_ln(float s) {
    return logf(s);
}
template <>
double mqi_ln(double s) {
    return log(s);
}

///< sqrt
template <>
float mqi_sqrt(float s) {
    return sqrtf(s);
}
template <>
double mqi_sqrt(double s) {
    return sqrt(s);
}

///< power
template <>
float mqi_pow(float s, float p) {
    return powf(s, p);
}
template <>
double mqi_pow(double s, double p) {
    return pow(s, p);
}

///< exponential
template <>
float mqi_exp(float s) {
    return expf(s);
}
template <>
double mqi_exp(double s) {
    return exp(s);
}

///< acos
template <>
float mqi_acos(float s) {
    return acosf(s);
}
template <>
double mqi_acos(double s) {
    return acos(s);
}

///< cos
template <>
float mqi_cos(float s) {
    return cosf(s);
}
template <>
double mqi_cos(double s) {
    return cos(s);
}
///< sin
template <>
float mqi_sin(float s) {
    return sinf(s);
}
template <>
double mqi_sin(double s) {
    return sin(s);
}

template <>
float mqi_abs(float s) {
    return abs(s);
}
template <>
double mqi_abs(double s) {
    return abs(s);
}

template <>
float mqi_round(float s) {
    return roundf(s);
}
template <>
double mqi_round(double s) {
    return round(s);
}

template <>
float mqi_floor(float s) {
    return floorf(s);
}
template <>
double mqi_floor(double s) {
    return floor(s);
}

template <>
float mqi_ceil(float s) {
    return ceilf(s);
}
template <>
double mqi_ceil(double s) {
    return ceil(s);
}

template <>
bool mqi_isnan(float s) {
    bool t = isnan(s);
    return t;
}
template <>
bool mqi_isnan(double s) {
    bool t = isnan(s);
    return t;
}
...
template <>
float mqi_uniform<float>(mqi_rng* rng) {
...
template <>
float mqi_normal<float>(mqi_rng* rng, float avg, float sig) {
    return curand_normal(rng) * sig + avg;
}

template <>
double mqi_normal<double>(mqi_rng* rng, double avg, double sig) {
    return curand_normal_double(rng) * sig + avg;
}

template <>
float mqi_exponential<float>(mqi_rng* rng, float avg, float up) {
    float x;
    do {
