...
/// \see http://dicom.nema.org/medical/dicom/current/output/chtml/part03/sect_C.8.8.26.html for
/// RTIBTR
...
typedef enum { NONE = 0, UNIFORM = 1, MODULATED = 2, MODULATED_SPEC = 3 } SCAN_MODE;
...
class beam_module_ion : public beam_module {
   ...
   public:
    ...
    const std::vector<int>* get_nb_spots_per_layer(void) const { return &nb_spots_per_layer_; }

    /// Returns a pointer of spot-sequence vector
    const std::vector<spot>* get_sequence(void) const { return &sequence_; }

    /// Returns tune-id
    const std::string get_tune_id(void) const { return tune_id_; }
