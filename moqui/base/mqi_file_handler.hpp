...
class mask_reader {
   public:
    ...
    CUDA_HOST
    uint8_t* read_mha_file(std::string filename) {
        std::string line;
    ...
    CUDA_HOST
    int32_t* mask_mapping(uint8_t* mask) {
    ...
    CUDA_HOST
    uint32_t size(int32_t* scorer_idx) {
        int count = 0;
        for (uint32_t ind = 0; ind < this->ct_size; ind++) {
            if (scorer_idx[ind] >= 0) {
                count++;
    ...
    CUDA_HOST
    void save_map(std::string filename, int32_t* scorer_idx) {
    ...
    CUDA_HOST
    void set_mask(uint8_t* mask) { this->mask_total = mask; }

    CUDA_HOST
    cnb_t ijk2cnb(ijk_t i, ijk_t j, ijk_t k) {
        return k * this->ct_dim.x * this->ct_dim.y + j * this->ct_dim.x + i;
    }
    CUDA_HOST
    mqi::roi_t* mask_to_roi() {
...
class file_parser {
   public:
    ...
    CUDA_HOST
    std::vector<std::string> read_input_parameters() {
    ...
    CUDA_HOST
    std::string get_path(std::string filepath) {
        std::string delimeter = "/";
        std::string path = "";
    ...
    CUDA_HOST
    std::string get_string(std::string option, std::string default_value) {
        size_t pos;
    ...
    CUDA_HOST
    std::vector<std::string> get_string_vector(std::string option, std::string delimeter1) {
        size_t pos_current = 0, pos_prev = 0;
    ...
    CUDA_HOST
    float get_float(std::string option, float default_value) {
        float value = std::atof(get_string(option, std::to_string(default_value)).c_str());
        return value;
    }

    CUDA_HOST
    std::vector<float> get_float_vector(std::string option, std::string delimter) {
    ...
    CUDA_HOST
    int get_int(std::string option, int default_value) {
        int value = std::atoi(get_string(option, std::to_string(default_value)).c_str());
        return value;
    }

    CUDA_HOST
    std::vector<int> get_int_vector(std::string option, std::string delimeter) {
        std::vector<std::string> value_tmp = get_string_vector(option, delimeter);
        if (value_tmp.size() > 0) {
    ...
    CUDA_HOST
    bool get_bool(std::string option, bool default_value) {
    ...
    CUDA_HOST
    scorer_t string_to_scorer_type(std::string scorer_name) {
    ...
    CUDA_HOST
    aperture_type_t string_to_aperture_type(std::string aperture_string) {
    ...
    CUDA_HOST
    sim_type_t string_to_sim_type(std::string sim_name) {
        sim_type_t type;
