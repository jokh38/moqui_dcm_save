...
class beamline {
   ...
   public:
    ...
    ~beamline() { geometries_.clear(); }

    /// Add new geometry to the container.
    void append_geometry(mqi::geometry* geo) { geometries_.push_back(geo); }

    /// Returns geometry container (const reference)
    const std::vector<mqi::geometry*>& get_geometries() { return geometries_; }
};
