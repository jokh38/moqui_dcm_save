...
class aperture : public geometry {
   ...
   public:
    ...
    aperture(std::vector<std::vector<std::array<float, 2>>> xypts, mqi::vec3<float>& v,
             mqi::vec3<float>& p, mqi::mat3x3<float>& r, bool is_rect = true)
        : block_data(xypts),
          volume(v),
          is_rectangle(is_rect),
          geometry(p, r, mqi::geometry_type::BLOCK) {
        ;
    }
    ...
    /// Assignment operator
    const aperture& operator=(const mqi::aperture& rhs) { return rhs; }
};
