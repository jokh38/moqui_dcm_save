...
/// This class has three user-implemented methods, create_beamline, create_beamsource, and
/// create_coordinate_transform \tparam T type of phase-space variables, e.g., float or double \note
/// treatment machine is distinguished in its name
template <typename T>
class treatment_machine {
   protected:
   ...
   public:
    ...
    /// \return beamline
    virtual mqi::beamline<T> create_beamline(const mqi::dataset* ds, mqi::modality_type m) = 0;
    ...
    // Log file based implementation for Samsung Medical Center by Chanil Jeon (2023-08-18)
    virtual mqi::beamsource<T> create_beamsource(const mqi::logfiles_t& logfileData,
                                                 const mqi::coordinate_transform<T> pcoord,
    ...
    virtual mqi::coordinate_transform<T> create_coordinate_transform(
        const mqi::dataset* ds, const mqi::modality_type m) = 0;
