...
class interaction {
   ...
   public:
    ...
    /// rho_mass (cm^3)
    CUDA_HOST_DEVICE
    virtual R sample_step_length(const relativistic_quantities<R>& rel, const material_t<R>& mat,
                                 mqi_rng* rng) {
    ...
    /// rho_mass (cm^3)
    CUDA_HOST_DEVICE
    virtual R sample_step_length(const R cs, mqi_rng* rng) {
        R mfp = (cs == 0.0) ? max_step : 1.0 / cs;
        R prob = mqi_uniform<R>(rng);
        return -1.0 * mfp * mqi_ln(prob);
    }
    ...
    CUDA_HOST_DEVICE
    virtual R cross_section(const relativistic_quantities<R>& rel, const material_t<R>& mat) = 0;
    ...
    virtual void post_step(track_t<R>& trk, track_stack_t<R>& stk, mqi_rng* rng, const R len,
                           material_t<R>& mat, bool score_local_deposit) = 0;
};
