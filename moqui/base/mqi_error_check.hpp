...
inline void check_cuda_last_error(const char* msg) {
    ...
    if (err != cudaSuccess) {
        ...
        printf("current total %f MB free %f MB\n", total / (1024.0 * 1024.0),
               free / (1024.0 * 1024.0));
...
inline void cuda_error_checker(cudaError_t code, const char* file, int line, bool abort = true) {
    if (code != cudaSuccess) {
