...
//  phase-space variables are position (x,y,z) and direction (x',y',z') between x_max, x_min and
//  y_max and y_min, etc.
...
class phsp_6d_fanbeam : public pdf_Md<T, 6> {
   ...
   public:
    ...
    CUDA_HOST_DEVICE
    phsp_6d_fanbeam(std::array<T, 6>& m,  ///< [x_min, x_max, y_min, y_max, z_min, z_max]
    ...
    /// Constructor to initialize distribution parameters
    CUDA_HOST_DEVICE
    phsp_6d_fanbeam(const std::array<T, 6>& m,  ///< [x_min, x_max, y_min, y_max, z_min, z_max]
    ...
    CUDA_HOST_DEVICE
    virtual std::array<T, 6> operator()(std::default_random_engine* rng) {
        auto x = unifx_(*rng);
        auto y = unify_(*rng);
        ...
        phsp[3] = dir.x + pdf_Md<T, 6>::sigma_[3] *
                              (rho_[0] * Ux + Vx * std::sqrt(1.0 - rho_[0] * rho_[0]));
        phsp[4] = dir.y + pdf_Md<T, 6>::sigma_[4] *
                              (rho_[1] * Uy + Vy * std::sqrt(1.0 - rho_[1] * rho_[1]));
