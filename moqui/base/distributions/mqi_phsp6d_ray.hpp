...
class phsp_6d_ray : public pdf_Md<T, 6> {
   ...
   public:
    ...
    CUDA_HOST_DEVICE
    phsp_6d_ray(std::array<T, 6>& m, std::array<T, 6>& s, std::array<T, 2>& r,
                float source_position)
    ...
    /// Constructor to initializes mean, sigma, rho, and random engine
    CUDA_HOST_DEVICE
    phsp_6d_ray(const std::array<T, 6>& m, const std::array<T, 6>& s, const std::array<T, 6>& r,
    ...
    //        //        phsp[3] +=
    //        //          pdf_Md<T, 6>::sigma_[3] * (rho_[0] * Ux + Vx * std::sqrt(1.0 - rho_[0] *
    //        rho_[0]));
    //        //        phsp[4] +=
    //        //          pdf_Md<T, 6>::sigma_[4] * (rho_[1] * Uy + Vy * std::sqrt(1.0 - rho_[1] *
    //        rho_[1])); phsp[5] = -1.0 * std::sqrt(1.0 - phsp[3] * phsp[3] - phsp[4] * phsp[4]);
    //        return phsp;
    //    };
};
