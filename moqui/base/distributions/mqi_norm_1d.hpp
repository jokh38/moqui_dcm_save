...
class norm_1d : public pdf_Md<T, 1> {
   public:
    ...
    CUDA_HOST_DEVICE
    norm_1d(std::array<T, 1>& m, std::array<T, 1>& s) : pdf_Md<T, 1>(m, s) {
    ...
    CUDA_HOST_DEVICE
    norm_1d(const std::array<T, 1>& m, const std::array<T, 1>& s) : pdf_Md<T, 1>(m, s) {
        //#if !defined(__CUDACC__)
        func_ = std::normal_distribution<T>(m[0], s[0]);
        //#endif
    }
    ...
    CUDA_HOST_DEVICE
    virtual std::array<T, 1> operator()(std::default_random_engine* rng) { return {func_(*rng)}; };
};
