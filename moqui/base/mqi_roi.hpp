...
class roi_t {
   ...
   public:
    CUDA_HOST_DEVICE
    roi_t(roi_mapping_t m, uint32_t n, int32_t l = 0, uint32_t* s = nullptr, uint32_t* t = nullptr,
          uint32_t* a = nullptr)
        : method_(m), original_length_(n), length_(l), start_(s), stride_(t), acc_stride_(a) {
        ;
    }

    CUDA_HOST_DEVICE
    int32_t idx(const uint32_t& v) const {
        switch (method_) {
            case INDIRECT:
                return start_[v];
            case CONTOUR:
    ...
    CUDA_HOST_DEVICE
    int32_t get_mask_idx(const uint32_t& v) const {
        switch (method_) {
            case INDIRECT:
                return start_[v];
            case CONTOUR:
    ...
    CUDA_HOST_DEVICE
    int32_t idx_contour(const uint32_t& v) const {
        int32_t c = this->lower_bound_cpp(v) - 1;
        uint32_t distance = v - start_[c];
        if (distance < stride_[c]) {
            /// is in stride
    ...
    CUDA_HOST_DEVICE
    int32_t idx_direct(const uint32_t& v) const { return start_[v]; }

    ///< Binary search
    CUDA_HOST_DEVICE
    int32_t lower_bound_cpp(const int32_t& value) const {
        int32_t first = 0;
