   ...
   public:
    ...
    virtual mqi::beamlet<T> characterize_beamlet(const mqi::beam_module_ion::spot& s0,
        ...
        mqi::vec3<T> dir0(std::atan(s0.x / treatment_machine_ion<T>::SAD_[0]),
                          std::atan(s0.y / treatment_machine_ion<T>::SAD_[1]), -1.0);
        mqi::vec3<T> dir1(std::atan(s1.x / treatment_machine_ion<T>::SAD_[0]),
                          std::atan(s1.y / treatment_machine_ion<T>::SAD_[1]), -1.0);
        ...
        // so passing spot-range to a specialized distribution for handling this case is prefered
        // option.
        auto spot_ = new mqi::phsp_6d_fanbeam<T>(spot_pos_range, spot_sigma, corr,
                                                 treatment_machine_ion<T>::SAD_);
    ...
    virtual mqi::beamlet<T> characterize_beamlet(const mqi::beam_module_ion::spot& s) {
        ...
        mqi::vec3<T> beam = this->beam_starting_position(
            iso, mqi::treatment_machine_ion<T>::source_to_isocenter_mm_);
    ...
    /// Characterize beamlet for MODULATED beam
    mqi::beamlet<T> characterize_beamlet(const mqi::beam_module_ion::spot& s,
    ...
    /// \param s_curr current spot to be delivered. The irradiation time depends on meterset of this
    /// spot \param s_next next spot to be delivered. To calculate time to move from this to next
    /// spot. dT_move
    virtual std::array<T, 2> characterize_beamlet_time(const mqi::beam_module_ion::spot& s_curr,
        ...
        if (&s_next != nullptr) {
            if (s_next.e != s_curr.e) {
                /// Layer changae.
                /// Note: The spot-settling time is not required or included in layer-switching
                /// time.
                ...
                if (dTx > 0 || dTy > 0)
                    dT_off = (dTy >= dTx) ? dTy + (time_spec_.set_y * ms)
                                          : dTx + (time_spec_.set_x * ms);
            }
    ...
    mqi::aperture* characterize_aperture(const mqi::dataset* ds, mqi::modality_type m) {
        ...
        mqi::vec3<float> lxyz(geometry_spec_.aperture[0], geometry_spec_.aperture[1],
                              ftmp[0]);  // 360 or 0?
    ...
    void load_beamdata(const std::string f) {
        ...
        while (std::getline(file, line)) {
            ...
            if (line.compare("[time]") == 0) {
                ...
                while (std::getline(file, line)) {
                    ...
                    T val;
                    data >> k >> val;
                    if (k.compare("E(sec)") == 0)
                        time_spec_.e = val;
                    if (k.compare("I(nA)") == 0)
                        time_spec_.i = val;
                    if (k.compare("dT_up(ms)") == 0)
                        time_spec_.dt_up = val;
                    if (k.compare("dT_down(ms)") == 0)
                        time_spec_.dt_down = val;
                    if (k.compare("set_x(ms)") == 0)
                        time_spec_.set_x = val;
                    if (k.compare("set_y(ms)") == 0)
                        time_spec_.set_y = val;
                    if (k.compare("velocity_x(m/sec)") == 0)
                        time_spec_.vx = val;
                    if (k.compare("velocity_y(m/sec)") == 0)
                        time_spec_.vy = val;
