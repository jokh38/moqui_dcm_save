...
///< Particle type
typedef enum { PHOTON = 0, ELECTRON = 1, PROTON = 2, NEUTRON = 3 } particle_t;
// PDG[PHOTON] =
// PDG[PROTON] = 2212 //
...
class track_t {
   public:
    ...
    ///< Constructor
    CUDA_HOST_DEVICE
    track_t(status_t s, process_t p, bool is_p, particle_t t, vertex_t<R> v0, vertex_t<R> v1,
            const R& dE)
        : status(s),
          process(p),
          primary(is_p),
    ...
    ///< Deposit energy
    CUDA_HOST_DEVICE
    void deposit(R e) { dE += e; }

    ///< Deposit energy
    CUDA_HOST_DEVICE
    void local_deposit(R e) { local_dE += e; }
    ...
    CUDA_HOST_DEVICE
    void shorten_step(R ratio)  // 0 < ratio < 1
    {
    ...
    ///<
    CUDA_HOST_DEVICE
    void update_post_vertex_direction(const R& theta, const R& phi) {
    ...
    CUDA_HOST_DEVICE
    void update_post_vertex_position(const R& len) { vtx1.pos = vtx0.pos + vtx0.dir * len; }

    ///<
    CUDA_HOST_DEVICE
    void update_post_vertex_energy(const R& e) { vtx1.ke -= e; }
...
template <typename R>
CUDA_HOST_DEVICE void assert_track(const mqi::track_t<R>& trk, int8_t id = -1) {
