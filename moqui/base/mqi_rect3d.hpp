...
class rect3d {
   ...
   protected:
    ...
    // R  x_edge[2]; /**< x boundary of entire rectlinear, e.g., x_edge[0] for x min, x_edge[1] for
    // x max*/ R  y_edge[2]; /**< y boundary of entire rectlinear, e.g., y_edge[0] for y min,
    // y_edge[1] for y max*/ R  z_edge[2]; /**< y boundary of entire rectlinear, e.g., z_edge[0] for
    // z min, z_edge[1] for z max*/
   ...
   public:
    ...
    virtual T operator[](const std::array<size_t, 3> p)  //&
    {
    ...
    virtual T operator[](const std::array<int, 3> p)  //&
    {
        return data_[ijk2data(p[0], p[1], p[2])];
    }
    ...
    virtual std::array<T, 8> cell_data(const std::array<size_t, 3>& c000_idx) {
    ...
    virtual std::array<R, 6> cell_position(const std::array<size_t, 3>& c000_idx) {
    ...
    CUDA_HOST
    virtual std::array<size_t, 3> find_c000_index(const mqi::vec3<R>& pos) {
    ...
    CUDA_HOST
    inline virtual size_t find_c000_x_index(const R& x) {
        // In case this code runs on GPU, consider to implement binary_search algorithm or use
        // thrust but thrust performance is not so good from
    ...
    CUDA_HOST
    inline virtual size_t find_c000_y_index(const R& y) {
    ...
    inline virtual size_t find_c000_z_index(const R& z) {
    ...
    CUDA_HOST_DEVICE
    inline virtual bool is_in_point(const vec3<R>& p) {
        ///< check p is inside of pixel grid not entire volume
        ///< Min/Max of src

        if (p.x < x_[0] || p.x >= x_[dim_.x - 1])
            return false;
        if (p.y < y_[0] || p.y >= y_[dim_.y - 1])
            return false;
        if (p.z < z_[0] || p.z >= z_[dim_.z - 1])
            return false;
    ...
    /// \return true if the p is in grid or false
    CUDA_HOST_DEVICE
    inline virtual bool is_in_rect(const vec3<R>& p) {
    ...
    CUDA_HOST_DEVICE
    mqi::vec3<R> get_center() {
        return mqi::vec3<R>(0.5 * (x_[0] + x_[dim_.x - 1]), 0.5 * (y_[0] + y_[dim_.y - 1]),
    ...
    CUDA_HOST_DEVICE
    void set_nxyz(mqi::vec3<ijk_t> dim) { dim_ = dim; }
    /// Returns position of the first corner cell
    CUDA_HOST_DEVICE
    mqi::vec3<R> get_origin() { return mqi::vec3<R>(x_[0], y_[0], z_[0]); }

    /// Prints out x,y,z coordinate positions
    CUDA_HOST
    virtual void dump_pts() {
        std::cout << "X: ";
        for (size_t i = 0; i < dim_.x; ++i) {
            std::cout << " " << x_[i] << " ";
        }
    ...
    CUDA_HOST_DEVICE
    virtual inline size_t ijk2data(size_t i, size_t j, size_t k) {
        return k * dim_.x * dim_.y + j * dim_.x + i;
    }

    /// Writes data into file
    CUDA_HOST
    virtual void write_data(const std::string filename) {
    ...
    template <class S>
    void write_data(std::valarray<S>& output, const std::string filename) {
        std::ofstream file1(filename, std::ios::out | std::ofstream::binary);
    ...
    */
    ...
    CUDA_HOST
    virtual void load_data() { data_.resize(dim_.x * dim_.y * dim_.z); }
    ...
    void read_data(T* src, size_t total) { data_(src, total); }

    void read_data(std::valarray<T> src) { data_ = src; }
    ...
    friend void warp_linear(rect3d<T0, R0>& src, rect3d<T0, R0>& ref,
                            rect3d<mqi::vec3<S0>, R0>& dvf, T0 fill_value);
};

/// R0 and R1 should be comparable
template <typename T0, typename R0, typename T1, typename R1>
void clone_structure(rect3d<T0, R0>& src, rect3d<T1, R1>& dest) {
    dest.dim_ = src.dim_;
...
/// R0 and R1 should be comparable
template <typename T0, typename R0, typename T1, typename R1>
void interpolate(rect3d<T0, R0>& src, rect3d<T1, R1>& dest, T1& fill_value) {
...
template <typename T0, typename R0, typename S0>
void warp_linear(rect3d<T0, R0>& src, rect3d<T0, R0>& dest, rect3d<mqi::vec3<S0>, R0>& vf,
                 T0 fill_value) {
    ///< Number of voxels: Nx, Ny, Nz
    const size_t nX = dest.dim_.x;
    const size_t nY = dest.dim_.y;
        ...
        for (size_t j = 0; j < nY; ++j) {
            ...
            for (size_t i = 0; i < nX; ++i) {
                ...
                if (vf.is_in_point(p_dest)) {
                    ...
                    /// Then, assign value by interpolating values at 8 coners surrounding new
                    /// position.
                    ///
