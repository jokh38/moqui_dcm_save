...
class treatment_machine_ion : public treatment_machine<T> {
   ...
   public:
    ...
    virtual mqi::coordinate_transform<T> create_coordinate_transform(const mqi::dataset* ds,
    ...
    ///
    virtual mqi::beamsource<T> create_beamsource(const mqi::dataset* ds, const mqi::modality_type m,
        ...
        for (size_t i = 0; i < nb_spots; ++i) {
            ...
            ///            coordinate system
            ///            beamlet time on/off
            beamsource.append_beamlet(this->characterize_beamlet(spots[i]), nb_histories, pcoord,
                                      time_on_off[0], time_on_off[1]);
        }
    ...
    /// \note maybe return const mqi::beamsource<T>& ?
    ///
    virtual mqi::beamsource<T> create_beamsource(
        const std::vector<mqi::beam_module_ion::spot>& spots, const mqi::modality_type m,
        const mqi::coordinate_transform<T> pcoord, const float particles_per_history = -1,
        const float source_to_isocenter_mm = 390.0) {
        treatment_machine<T>::source_to_isocenter_mm_ = source_to_isocenter_mm;
        ///< Parse DICOM beam module for ION
        mqi::beamsource<T> beamsource;
        ...
        for (size_t i = 0; i < nb_spots; ++i) {
            ...
            //// TODO: history counting is different from the other method and would be very
            /// confisuing
            size_t nb_histories = (particles_per_history <= 1)
                                      ? 1
    ...
    virtual mqi::beamlet<T> characterize_beamlet(const mqi::beam_module_ion::spot& s) = 0;
    ...
    virtual mqi::beamlet<T> characterize_beamlet(const mqi::beam_module_ion::logspot& s,
    ...
    virtual std::array<T, 2> characterize_beamlet_time(const mqi::beam_module_ion::spot& s_current,
    ...
    virtual size_t characterize_history(const mqi::beam_module_ion::spot& s, float scale) {
        return s.meterset / scale;
    }

    /// User method to characterize number of histories per UNIFORM/MODULATED_SPEC beamlet
    /// based on spot information from DICOM.
    virtual size_t characterize_history(const mqi::beam_module_ion::spot& s0,
                                        const mqi::beam_module_ion::spot& s1, float scale) {
    ...
    virtual mqi::beamline<T> create_beamline(const mqi::dataset* ds, mqi::modality_type m) {
        ...
        std::cout << "Creating beam line object.. : Number of range shifter sequence --> "
                  << itmp[0] << std::endl;
        if (itmp[0] >= 1) {
            beamline.append_geometry(this->characterize_rangeshifter(ds, m));
        }
        ...
        if (itmp[0] >= 1) {
            beamline.append_geometry(this->characterize_aperture(ds, m));
        }
        return beamline;
    }
    ...
    virtual mqi::rangeshifter* characterize_rangeshifter(const mqi::dataset* ds,
                                                         mqi::modality_type m) = 0;

    /// Returns aperture
    /// \param ds one-item of IonBeamSequence
    /// \param m  modality type, e.g., RTIP or RTIBTR
    /// \note Users need to implement their own rule to convert DICOM info to RS specification.
    virtual mqi::aperture* characterize_aperture(const mqi::dataset* ds, mqi::modality_type m) = 0;
    ...
    const std::vector<std::vector<std::array<float, 2>>> characterize_aperture_opening(
        const mqi::dataset* ds, mqi::modality_type m) {
    ...
    virtual mqi::vec3<T> beam_starting_position(const mqi::vec3<T>& iso, T z) {
        mqi::vec3<T> beam(0, 0, z);
        beam.x = iso.x * (treatment_machine_ion<T>::SAD_[0] - beam.z) /
                 treatment_machine_ion<T>::SAD_[0];
        beam.y = iso.y * (treatment_machine_ion<T>::SAD_[1] - beam.z) /
                 treatment_machine_ion<T>::SAD_[1];

        // std::cout << "Beam starting position : X --> " << beam.x << " Y --> " << beam.y << " Z
        // --> " << beam.z << std::endl;
