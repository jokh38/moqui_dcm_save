...
class mat3x3 {
   public:
    ...
    CUDA_HOST
    vec3<T> euler_xyz(bool y_is_2nd_quad = false) {
        vec3<T> psi_th_phi;
    ...
    CUDA_HOST_DEVICE
    std::array<T, 3> operator*(const std::array<T, 3>& r) const {
    ...
    vec3<T> operator*(const vec3<T>& r) const {
        return vec3<T>(xx * r.x + xy * r.y + xz * r.z, yx * r.x + yy * r.y + yz * r.z,
    ...
    CUDA_HOST_DEVICE
    mat3x3<T> operator*(const mat3x3<T>& r) const {
        return mat3x3<T>(xx * r.xx + xy * r.yx + xz * r.zx, xx * r.xy + xy * r.yy + xz * r.zy,
                         xx * r.xz + xy * r.yz + xz * r.zz, yx * r.xx + yy * r.yx + yz * r.zx,
                         yx * r.xy + yy * r.yy + yz * r.zy, yx * r.xz + yy * r.yz + yz * r.zz,
                         zx * r.xx + zy * r.yx + zz * r.zx, zx * r.xy + zy * r.yy + zz * r.zy,
...
class mat4x4 {
   public:
    ...
    CUDA_HOST_DEVICE
    mat4x4(T xx, T xy, T xz, T xs, T yx, T yy, T yz, T ys, T zx, T zy, T zz, T zs, T sx, T sy, T sz,
           T ss)
        : xx(xx),
          xy(xy),
          xz(xz),
    ...
    CUDA_HOST_DEVICE
    std::array<T, 4> operator*(const std::array<T, 4>& r) const {
    ...
    vec4<T> operator*(const vec4<T>& r) const {
        return vec4<T>(
            xx * r.x + xy * r.y + xz * r.z + xs * r.s, yx * r.x + yy * r.y + yz * r.z + ys * r.s,
            zx * r.x + zy * r.y + zz * r.z + zs * r.s, sx * r.x + sy * r.y + sz * r.z + ss * r.s);
    }

    CUDA_HOST_DEVICE
    vec3<T> operator*(const vec3<T>& r) const {
        return vec3<T>(xx * r.x + xy * r.y + xz * r.z + xs, yx * r.x + yy * r.y + yz * r.z + ys,
                       zx * r.x + zy * r.y + zz * r.z + zs);
    }

    CUDA_HOST_DEVICE
    mat4x4<T> operator*(const mat4x4<T>& r) const {
