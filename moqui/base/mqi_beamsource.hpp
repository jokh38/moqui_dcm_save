...
class beamsource {
   public:
    ...
    void append_beamlet(mqi::beamlet<T> b, size_t h, mqi::coordinate_transform<T> p, T time_on = 1,
                        T time_off = 0) {
    ...
    void append_beamlet_log(mqi::beamlet<T> b, size_t h, mqi::coordinate_transform<T> p) {
    ...
    /// \param b a beamlet
    /// \param h number of histories
    void append_beamlet(mqi::beamlet<T> b, size_t h, T time_on = 1, T time_off = 0) {
        const size_t acc = total_histories() + h;
        const size_t beamlet_id =
            this->total_beamlets();  // current number of beamlets -> beamlet ID
        cdf2beamlet_.insert(std::make_pair(acc, beamlet_id));
    ...
    const std::tuple<mqi::beamlet<T>, size_t, size_t>& operator[](unsigned int i) const {
        return beamlets_[i];
    }

    /// Returns a beamlet of a history
    /// \return a beamlet reference (const)
    const mqi::beamlet<T>& operator()(size_t h) {
    ...
    /// \param  time
    size_t cumulative_history_at_time(T t0) {
        auto pulse = timeline_.lower_bound(t0);
        if (pulse == timeline_.end())
            return this->total_histories();
